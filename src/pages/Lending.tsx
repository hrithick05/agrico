...
const loanCircles = [
  {
    id: 1,
    name: "Tamil Nadu Rice Farmers Circle",
    description: "Supporting Ponni rice cultivation in Thanjavur delta region",
  ...
  {
    id: 2,
    name: "Coimbatore Cotton Growers",
    description: "Equipment and input financing for cotton farmers in Coimbatore",
    status: "active" as const,
    members: 8,
    maxMembers: 10,
    interestRate: 9,
  ...
  {
    id: 3,
    name: "Madurai Vegetable Farmers",
    description: "Quick loans for vegetable farming needs in Madurai region",
  ...
  {
    id: 4,
    name: "Salem Dairy Farmers Circle",
    description: "Cattle purchase and dairy equipment loans in Salem district",
    status: "new" as const,
    members: 6,
    maxMembers: 12,
    interestRate: 8.5,
...
const initialMyLoans: Loan[] = [
  {
    id: 1,
    amount: 25000,
    status: 'approved',
    purpose: 'Ponni rice seeds for Thanjavur delta cultivation',
    circle: 'Tamil Nadu Rice Farmers Circle',
    dueDate: 'June 5, 2024',
    monthlyEMI: 8750
  },
  {
    ...
    amount: 15000,
    status: 'pending',
    purpose: 'Cotton fertilizer and pesticides for Coimbatore fields',
    circle: 'Coimbatore Cotton Growers',
  ...
  },
  {
    ...
    amount: 10000,
    status: 'repaid',
    purpose: 'Vegetable farming tools and irrigation equipment',
    circle: 'Madurai Vegetable Farmers',
...
const MicroLending: React.FC = () => {
  ...
  const [currentUser, setCurrentUser] = useState<User>(() => {
    // Try to get user name from localStorage, fallback to default
    const savedUser = localStorage.getItem('lendingUser');
    if (savedUser) {
      try {
        ...
        return {
          ...
          joinedCircles: parsedUser.joinedCircles || [],
  ...
  const [circles, setCircles] = useState<Circle[]>(loanCircles);
  const [loanRequests, setLoanRequests] = useState<LoanRequest[]>([]);
  const [myLoans, setMyLoans] = useState<Loan[]>(() => {
    // Try to get saved loans from localStorage
  ...
  useEffect(() => {
    localStorage.setItem('lendingUser', JSON.stringify(currentUser));
  }, [currentUser]);

  // Save user loans to localStorage
  useEffect(() => {
    localStorage.setItem('userLoans', JSON.stringify(myLoans));
  }, [myLoans]);
...
Admin: ${selectedCircleData.adminName} (${selectedCircleData.adminPhone})`;
    ...
    // Add to admin panel requests
    setLoanRequests(prev => [...prev, newRequest]);
    
    // Add to user's personal loans list
    const newLoan: Loan = {
      id: newRequest.id,
      ...
      circle: selectedCircleData.name,
      appliedDate: newRequest.requestDate
    };
    
    // Update myLoans state
  ...
  const handleAdminDecision = (requestId: number, decision: 'approved' | 'rejected') => {
    ...
    // Update the corresponding loan in user's loan list
        ...
        prev.map(loan => 
          loan.id === requestId 
            ? { 
                ...loan, 
                status: 'approved',
                dueDate: 'June 5, 2024', // You can make this dynamic
                monthlyEMI: Math.round(loan.amount / 3) // Simple EMI calculation
              } 
            : loan
        )
      );
       ...
       <div className="relative z-10 p-6">
         ...
         <div className="mb-8">
           <div className="flex justify-between items-start mb-4">
             <div>
               <h1 className="text-4xl font-bold text-white mb-2">{t('lending.title')}</h1>
               <p className="text-green-100 text-lg">{t('lending.subtitle')}</p>
             </div>
             <div className="text-right">
               <div className="bg-white bg-opacity-10 rounded-lg p-4 backdrop-blur-sm">
                 <div className="text-white text-sm mb-1">Welcome,</div>
                 <div className="text-white font-semibold text-lg mb-2">{currentUser.name}</div>
                 <button
        ...
        {currentPage === 'home' && (
          <div className="space-y-8">
            ...
            <div className="bg-white rounded-lg shadow-lg p-8 text-center">
              <h2 className="text-3xl font-bold text-gray-800 mb-4">{t('lending.welcomeTitle')}</h2>
              <p className="text-lg text-gray-600 mb-6">{t('lending.welcomeSubtitle')}</p>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div className="bg-green-50 rounded-lg p-6">
                  <div className="text-4xl mb-4">👥</div>
                  <h3 className="text-xl font-semibold text-gray-800 mb-2">{t('lending.joinCircle')}</h3>
                  <p className="text-gray-600">{t('lending.joinCircleDesc')}</p>
                  </div>
                <div className="bg-blue-50 rounded-lg p-6">
                  <div className="text-4xl mb-4">💰</div>
                  <h3 className="text-xl font-semibold text-gray-800 mb-2">{t('lending.requestLoan')}</h3>
                  <p className="text-gray-600">{t('lending.requestLoanDesc')}</p>
                  </div>
                <div className="bg-yellow-50 rounded-lg p-6">
                  <div className="text-4xl mb-4">🤝</div>
                  <h3 className="text-xl font-semibold text-gray-800 mb-2">{t('lending.buildTrust')}</h3>
                  <p className="text-gray-600">{t('lending.buildTrustDesc')}</p>
                </div>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
              <div className="bg-white rounded-lg shadow-lg p-6 text-center">
                ...
                <div className="text-gray-600">{t('lending.activeCircles')}</div>
              </div>
              <div className="bg-white rounded-lg shadow-lg p-6 text-center">
                <div className="text-3xl font-bold text-blue-600 mb-2">41</div>
                <div className="text-gray-600">{t('lending.totalMembers')}</div>
              </div>
              <div className="bg-white rounded-lg shadow-lg p-6 text-center">
                <div className="text-3xl font-bold text-yellow-600 mb-2">156</div>
                <div className="text-gray-600">{t('lending.loansCompleted')}</div>
              </div>
              <div className="bg-white rounded-lg shadow-lg p-6 text-center">
                <div className="text-3xl font-bold text-purple-600 mb-2">₹2.5M</div>
                <div className="text-gray-600">{t('lending.totalLending')}</div>
              </div>
            </div>
          </div>
        )}
        ...
        {currentPage === 'circles' && (
          <div className="space-y-6">
            <div className="bg-white rounded-lg shadow-lg p-6">
              <h2 className="text-2xl font-bold text-gray-800 mb-6">{t('lending.availableCircles')}</h2>
      ...
      {showProfileModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-lg shadow-xl max-w-md w-full p-6">
            ...
            <form onSubmit={(e) => { e.preventDefault(); handleUpdateUserName(); }} className="space-y-4">
              <div>
